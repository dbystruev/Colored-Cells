...
extension ColoredViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let components = color.cgColor.components else { return cell }
        guard components.count > 2 else { return cell }
        
        let red = lroundf(Float(255 * components[0]))
        let green = lroundf(Float(255 * components[1]))
        let blue = lroundf(Float(255 * components[2]))
        
        // Print in HEX
        cell.textLabel?.text = String(format: "#%02lX%02lX%02lX", red, green, blue)
        
        let isDarkColor = 299 * red + 587 * green + 114 * blue < 500 * 255
        
        cell.textLabel?.textColor = isDarkColor ? .lightText : .darkText
        return cell
